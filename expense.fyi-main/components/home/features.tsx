				...
				{features.map((feature, index) => {
					...
					return (
						<div key={`${feature.name}-${index}`}>
							<button
								className={
									'flex w-full justify-between rounded-lg bg-green-100 px-4 py-3 text-left text-sm font-medium text-green-900 hover:bg-green-200 focus:outline-none focus-visible:ring focus-visible:ring-green-500 focus-visible:ring-opacity-75'
								}
								onClick={() => {
									setSelected(index);
								}}
							>
								<div className="flex w-full items-center justify-between ">
									<div className="flex w-full items-center justify-between">
										...
										{
											<ChevronUpIcon
												className={`${isSelected ? 'rotate-180 transform' : ''} h-5 w-5 text-green-600`}
											/>
										}
									</div>
								</div>
